...
class RecommendedRepository @Inject constructor(
    ...
    suspend fun getRecommendedData(): Result<List<Service>, DataError> {
        return try {
            val serviceRecommended = firestoreDataSource.fetchRecommended()
            if (serviceRecommended.isNotEmpty()) {
                Result.Success(serviceRecommended)
            } else {
